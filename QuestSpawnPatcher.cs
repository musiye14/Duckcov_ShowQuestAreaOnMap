    ...
    namespace ShowQuestsAreaOnMap
    {
        public class ActiveQuestSpawn
        {
            ...
            public string TargetSubSceneID { get; set; }
        }
        ...
        public static class QuestSpawnPatcher
        {
            ...
            private static FieldInfo _spawnPrefab_LocationsField;
            ...
            public static void ApplyPatches(Harmony harmony)
            {
                ...
                _spawnPrefab_LocationsField = typeof(SpawnPrefabForTask)
                    .GetField("locations", BindingFlags.NonPublic | BindingFlags.Instance);
                if (_spawnPrefab_LocationsField == null)
                    Debug.LogError(LogPrefix + "反射失败: SpawnPrefabForTask.locations");
            ...
            private static void OnItemSpawned(SpawnItemForTask __instance, Vector3 pos, MultiSceneLocation location)
            {
                ...
                string subSceneId = location.SceneID;
           ...
           private static void OnInstantiate_Postfix(Object __result, Vector3 position)
           {
               if (_currentlySpawningPrefabTask != null && __result != null && __result is GameObject spawnedGameObject) 
               {
                   ...
                   Task task = null;
                   List<MultiSceneLocation> locations = null;
                   try
                   {
                       task = (Task)_spawnPrefabTaskTaskProperty.GetValue(instance, null);
                       locations = (List<MultiSceneLocation>)_spawnPrefab_LocationsField?.GetValue(instance);
                   }
                   catch (System.Exception ex)
                   {
                       Debug.LogError(LogPrefix + $"[OnInstantiate] 反射 task 失败: {ex.Message}"); return;
                   }
                   ...
                   if (locations != null)
                   {
                       ...
                       {
                           if(loc.IsUnityNull() && loc.TryGetLocationPosition(out Vector3 locPos) && Vector3.Distance(locPos, pos) < 0.1f)
                           {
                               subSceneId = loc.SceneID;
                               Debug.Log(LogPrefix + $"[OnInstantiate] 找到匹配的 MultiSceneLocation, SubSceneID: {subSceneId}");
                               break; 
                           }
                       }
                       if(subSceneId == null) Debug.LogWarning(LogPrefix + $"[OnInstantiate] 未能在 locations 列表中为位置 {pos} 找到匹配的 MultiSceneLocation。");
                   }
                   else
                   {
                       Debug.LogWarning(LogPrefix + "[OnInstantiate] 反射 locations 列表失败或列表为空。");
                   }
                   
                   if (ActiveSpawns.Exists(s => s.AssociatedTask == task && s.TargetSubSceneID == subSceneId && Vector3.Distance(s.Position, pos) < 0.1f)) return;
